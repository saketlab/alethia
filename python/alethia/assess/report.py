...
def _render_figure(draw) -> str | None:
    """Run ``draw(plt)`` and return its figure as a base64 PNG data URI."""
    plt = _matplotlib()
    if plt is None:
        return None
    fig = draw(plt)
...
def _ranking_figure(report: "AssessmentReport") -> str | None:
    def draw(plt):
...
def _alignment_uniformity_figure(report: "AssessmentReport") -> str | None:
    def draw(plt):
...
<li><b>Hubness</b>: skew of how often references are top-k neighbours; high values indicate
    a retrieval pathology. After Radovanovic et al. (2010),
