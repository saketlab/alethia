...
def filter_huggingface_only(mteb_df: pd.DataFrame) -> pd.DataFrame:
    """Drop non-HuggingFace rows from an MTEB DataFrame."""
    if mteb_df.empty:
        return mteb_df
...
def load_mteb_dashboard_data() -> pd.DataFrame:
    """MTEB dashboard data from package resources, cleaned and HuggingFace-filtered."""
    try:
...
def classify_embedding_models() -> dict[str, dict[str, Any]]:
    """Group models into recommendation categories by their characteristics."""
...
def create_recommendation_matrix() -> dict[str, list[str]]:
    """Map each use case to its list of recommended model names."""
    recommendations = {
...
def get_detailed_model_info() -> dict[str, dict[str, Any]]:
    """Per-model organization, size, parameters and characteristics."""
    model_details = {
        "all-MiniLM-L6-v2": {
...
def get_model_recommendation(use_case: str, constraint: str = None) -> list[str]:
    """Recommended model names for ``use_case``."""
    recommendations = create_recommendation_matrix()
